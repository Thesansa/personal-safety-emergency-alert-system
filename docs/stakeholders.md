...
## External Stakeholders (Future Scope)
Emergency services  
...
Trigger an SOS alert quickly to notify trusted contacts in an emergency.

Remain logged into the system to allow rapid SOS activation.

Cancel an active SOS alert to prevent false or accidental notifications.

Share my real-time location during an active SOS alert.

Add and manage multiple trusted contacts.

View the current status of an SOS alert.

Confirm safety and resolve the alert once the emergency is over.

## Trusted Contact
As a Trusted Contact, I should be able to:

Receive an emergency alert immediately when the user triggers an SOS.

Receive an escalation alert if the user becomes unresponsive.  

View the user’s real-time location during an active SOS alert.  

Acknowledge the emergency alert.  

View the current status of the SOS alert.  

Confirm or request resolution once the user is confirmed safe.  

View basic emergency event details for awareness.
...
Exit Conditions: User is authenticated User session is active
...
Entry Conditions: User is logged in At least one trusted contact is configured
...
Exit Conditions: SOS alert is activated Emergency notifications are sent to trusted contacts Location sharing and escalation timer are started
...
Exit Conditions: SOS alert status is changed to cancelled Location tracking is stopped Trusted contacts are notified of cancellation
...
Primary Actor: Trusted Contact Secondary Actor: System

Entry Conditions: User has triggered an SOS alert Trusted contact is registered in the system
...
Entry Conditions: SOS alert is active Trusted contact has received the alert
...
Entry Conditions: SOS alert is active Trusted contact has reached the user or believes the situation is resolved
...
Exit Conditions: Resolution request is recorded User is prompted to confirm safety SOS alert remains active until user confirmation or system-defined fallback occurs

## UC-10: Escalate SOS Alert
Primary Actor: System Secondary Actor: —

Entry Conditions: SOS alert is active User does not respond within the defined time period
...
Exit Conditions: Alert escalation is triggered Trusted contacts receive escalation notification
